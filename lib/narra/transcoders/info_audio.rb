...
module Narra
  module Transcoders
    class InfoAudio < Narra::SPI::Transcoder

      # Set title and description fields
      @identifier = :info_audio
      @title = 'Info Audio Transcoder'
      @description = 'Info Audio Transcoder based on ffmpeg'

      def self.valid?(item_to_check)
        item_to_check.type.equal?(:audio)
      end
